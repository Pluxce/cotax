...
import { CONTRIBUABLES as COMMERCANTS, formatAmount, type Contribuable as Commercant } from '@/lib/mock-data'
...
export function CommercantTable() {
  const [activeTab, setActiveTab] = useState('Tous')
  const [selected, setSelected] = useState<Commercant | null>(null)

  const filtered = activeTab === 'Tous' ? COMMERCANTS
    : activeTab === 'À jour' ? COMMERCANTS.filter(c => c.status === 'ok')
    : activeTab === 'En retard' ? COMMERCANTS.filter(c => c.status === 'late')
    : activeTab === 'En attente' ? COMMERCANTS.filter(c => c.status === 'pending')
    : COMMERCANTS
            ...
            <table className="w-full border-collapse" style={{ minWidth: 640 }}>
              <thead>
                <tr>
                  ...
                  {['Commerçant', 'Étal · Catégorie', 'Dû ce mois', 'Dernier paiement', 'Statut', ''].map((h, i) => (
                    <th
                      key={i}
